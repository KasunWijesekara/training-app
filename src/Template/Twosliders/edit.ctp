...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Form->postLink(
                __('Delete'),
                ['action' => 'delete', $twoslider->id],
                ['confirm' => __('Are you sure you want to delete # {0}?', $twoslider->id)]
            )
        ?></li>
...
<div class="twosliders form large-9 medium-8 columns content">
    ...
    <fieldset>
        <legend><?= __('Edit Twoslider') ?></legend>
        <?php
            echo $this->Form->input('image', ['type' => 'file', 'accept'=> '.png, .jpg, .jpeg']);
            echo $this->Form->control('status');
        ?>
    </fieldset>
    <input type="hidden" name="blog_imgold" value="<?php echo $twoslider->image; ?>">
